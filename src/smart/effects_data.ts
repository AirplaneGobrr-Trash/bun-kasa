/** Built-in light effect presets for SMART (Tapo) light strips. Ported from kasa/smart/effects.py. */
export type SmartLightEffectPreset = Record<string, unknown> & { name: string };

export const EFFECTS_LIST: SmartLightEffectPreset[] = [
  {
    custom: 0,
...
] as SmartLightEffectPreset[];

export const EFFECT_NAMES: string[] = EFFECTS_LIST.map((effect) => effect.name);

export const EFFECT_MAPPING: Record<string, SmartLightEffectPreset> = Object.fromEntries(
  EFFECTS_LIST.map((effect) => [effect.name, effect]),
);
