/** Built-in light-strip effect presets (KL430 and similar). Ported from kasa/iot/effects.py. */
export type IotLightEffectPreset = Record<string, unknown> & { name: string };

export const EFFECTS_LIST_V1: IotLightEffectPreset[] = [
  {
    custom: 0,
...
] as IotLightEffectPreset[];

export const EFFECT_NAMES_V1: string[] = EFFECTS_LIST_V1.map((effect) => effect.name);

export const EFFECT_MAPPING_V1: Record<string, IotLightEffectPreset> = Object.fromEntries(
  EFFECTS_LIST_V1.map((effect) => [effect.name, effect]),
);
