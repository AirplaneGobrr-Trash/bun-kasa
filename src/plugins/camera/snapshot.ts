...
Camera.prototype.getSnapshot = function (
  ...
  if (!rtspUrl) {
    return Promise.reject(
      new Error("getSnapshot requires local-account credentials and the camera to be on"),
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    (async () => {
      try {
        ...
        if (code !== 0) {
          reject(new Error(`ffmpeg exited with code ${code} while capturing a snapshot`));
          return;
        }
